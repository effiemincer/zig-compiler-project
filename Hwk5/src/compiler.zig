...
/// Jack-to-VM compiler.
/// Parses tokens generated by the tokenizer and emits VM code.
/// Handles all Jack grammar rules including class declarations, subroutines,
/// variable declarations, expressions, and control flow statements.
/// Manages variable scoping with a symbol table and writes formatted VM output.
